...
`/herdr` skill 执行。child subagents 的派发通过 `Agent` tool 执行（`run_in_background: true`）。
...
- gate/decision child consumed（pane runtime）：通过 `/herdr` skill 关闭 pane并确认消失后写入 `closed`。
