...
**Coordinator Runtime**
The current session hosting the coordinator: `codex-cli`, `claude-cli`, or `pi-cli` for canonical CLI/Herdr orchestration; `codex-app` only when the `delivery-pipeline-codex-app` shell is invoked; the current Orca terminal only when the independent Orca entrypoint is invoked. The current calling session is the coordinator; its model is chosen before skill invocation and is not part of Worker Task Configuration; a `coordinator` work entry only records recommendation observations.
