...
The worker-emitted single-line `LANE_DONE <lane_id>` marker required by the dispatch packet at both completed and blocked terminal states. The coordinator-side lane watcher (`skills/delivery-pipeline-herdr/scripts/lane-watch.sh`) polls the worker pane output for the marker and prompts the Coordinator Pane; pane-vanished and two-hour timeout also wake the coordinator. `herdr agent wait --until done` is not a valid terminal signal: CLI agents settle back to idle without emitting a `done` event, so a `done` listener blocks forever. HITL lanes still wake on the user completion signal; the watcher is only a terminal backstop.
...
**Orca Entrypoint**
The independent `delivery-pipeline-orca` skill bound to the current Orca runtime. Before any operation it fixes one executable and reads that binary's version-matched guides with exact native command provenance, runtime/target-host/terminal identity, exact skill discovery, shared worker policy, and operation-scoped capability evidence. Missing or Unknown evidence returns `blocked` / `dispatch unavailable`; it never falls back to Herdr or Codex App. #120 owns only this entry/preflight boundary; Orca lane lifecycle coordinates belong to later implementation tickets.

**Configured Planning Lane**
A Herdr worker handling one AFK discovery/research item or one spec/tickets gate. Its agent, model, and effort come from the `planning` task type in the Worker Task Configuration; it uses the same registry, packet, Dispatch Handoff, and fan-in contract as other configured lanes.
...
One shared config schema (version 4) with two file instances: the CLI instance at user-level `~/.config/delivery-pipeline/model-roles.json` and the App instance at `skills/delivery-pipeline-codex-app/config/models.json`. The schema defines task types (`coordinator`, `research`, `prototype`, `planning`, `design`, `frontend`, `backend`, `testing`, `integration`, `assistance`, `second-opinion`, `ticket-sizing`), each bound to a non-empty `agent`, `model`, and `effort`; agent is one of `pi`, `codex`, `claude`, or `codex-app` and selects the transport. Named `modes` presets carry `kind` (`staged`/`direct`) plus per-agent `starting`/`execution`/`direct` model/effort pairs; only implementation task types with `commit` output consume them, resolved in ticket → map → `default_mode` order. Formal review uses the `review.<implementation|whole-change>.<standards|spec>` matrix. The single schema authority is `skills/delivery-pipeline/references/model-config-schema.md`; each transport declares its required task-type subset. Canonical CLI skills contain no default model routing; missing, unknown, or invalid configuration blocks dispatch and runs `delivery-pipeline-setup`. Older config versions migrate mechanically via `model_config.py migrate`. Existing lanes recover from registry and are not migrated by a configuration change.

**Coordinator Runtime**
The current session hosting the coordinator: `codex-cli`, `claude-cli`, or `pi-cli` entering through the `delivery-pipeline-herdr` shell; `codex-app` only when the `delivery-pipeline-codex-app` shell is invoked; the current Orca terminal only when the `delivery-pipeline-orca` entrypoint is invoked. The current calling session is the coordinator; its model is chosen before skill invocation and is not part of Worker Task Configuration; a `coordinator` work entry only records recommendation observations.
...
- Canonical CLI Dispatch resolves work item → task type → configured agent/model/effort. Agent selects lane runtime: pi → `herdr-pi-pane`, codex → `herdr-codex-pane`, claude → `herdr-claude-pane`.
...
- `skills/delivery-pipeline-codex-app` remains the Codex App transport shell. Its instance of the Worker Task Configuration binds task types to `agent: codex-app` and maps delegated work to Codex App tasks + App-managed Execution Worktrees (`codex-thread`); its packet, registry overlay, task-aware fan-in, and task references are co-located inside that shell. App 模型值统一读取 `skills/delivery-pipeline-codex-app/config/models.json`；`references/development-mode.md` 定义查询与执行合同。新 lane 冻结计划与来源，恢复沿持久值；App execution readback remains separate from requests.
- `skills/delivery-pipeline-orca` is a separate Orca-bound entrypoint. It reuses the CLI version 4 schema and owner contract, but resolves one Orca executable and requires version-matched guides, exact native discovery, current terminal identity, shared agent/model/effort readback, and operation-scoped capabilities. It owns no fallback, override config, static compatibility matrix, second installer, or dispatcher.
- New implementation lanes resolve a named execution mode in ticket → map → `default_mode` order and freeze mode name, source, and phase model/effort in the packet and registry. Existing lanes recover from their registry and are never migrated by a config change.
- `delivery-pipeline-setup` probes pi (`pi --list-models`), Codex (`codex debug models`), and Claude `settings.json.env`, then requires explicit choices for all CLI-required task types and the review matrix. There are no built-in agent/model/effort defaults.
...
**Entrypoint Layering**
The rule that arranges the four entry skills. `skills/delivery-pipeline` is the Core Chain: the single home of shared semantics (gate state machine, lane registry, Worker Task Configuration schema, owner resolution) and of the pi/Codex CLI/Claude CLI coordinator adapters. `skills/delivery-pipeline-herdr`, `skills/delivery-pipeline-codex-app`, and `skills/delivery-pipeline-orca` are peer Transport Shells: each owns its channel-specific dispatch, monitoring, and lifecycle material, and reads the Core Chain. Dependencies point only from shell to core; the core may name shells and enumerate their runtime markers in shared state (the lane registry must parse lanes written by any runtime), but never references files inside a shell. Channel-specific content lives in its shell; shared content settles in the core; no fourth shared location exists.
...
- One execution worktree → one Codex App task or one Herdr-hosted Codex CLI/Claude Code/pi pane; future Orca Task/Dispatch ownership is not introduced by the #120 preflight entrypoint
- One Orca entry invocation → one fixed executable + one current terminal identity + one operation-scoped capability verdict
