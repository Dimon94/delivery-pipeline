...
1. 当前 Codex App 会话就是 coordinator，记录 `coordinator_runtime: codex-app`、
   ...
   当前 coordinator 推荐 `gpt-5.6-sol` / `high`，用户明确选择可覆盖所有阶段的模型与档位。
...
3. canonical 六个 role 与 output mode 保持不变。planning、design、frontend、backend 使用
   ...
   - testing → `output_mode: checks`，由 coordinator 串行委派 Luna / max 只读 subagent
   ...
   Integration 由 coordinator 单独串行委派 Luna / max 可写 subagent，并由 coordinator 回读 Git。
4. 外层 task roles 使用本壳 `assets/APP_ROLE_DISPATCH_PACKET.md`；创建、恢复、startup
   ...
   `review_scope: implementation | whole-change` 调用 resolved code-review owner；App 壳不选择
   owner/model/effort，实施 worker 的自评不能放行。候选 commit 在 Integration 前必须通过
