...
所有 Execution Worktree 从 dispatch 时 Integration HEAD 开始；Source Worktree保持原 branch；
每个 work item 只有一个 active writer。
...
创建前验证 Source repo/branch/HEAD/clean state 与 map registry；路径/branch 已存在时 bounded
recover，不覆盖。创建后验证 registration、common dir、branch、HEAD 与 clean state，写入 registry
并 readback。Herdr Workspace 到第一条 configured lane 才懒创建。
...
4. registry 写 role、agent、model、effort、runtime、worktree、branch、base commit，readback 后才创建 pane。
