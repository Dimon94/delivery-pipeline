...
def check_core_contract() -> None:
    ...
    require(
        ...
        (
            ...
            "Two-phase Bootstrap Startup",
            "Armed Projection readback 后才生成最终 packet",
        ),
    )
    ...
    require(
        ...
        (
            ...
            "Armed Projection",
            "最终 packet",
            "gearshift_state: requested",
            "Legacy v2 Replacement Checkpoint",
            "重新计算 packet SHA-256",
            "与 `packet_sha256` 精确相等",
            "首次 requested/phase-A start 不要求 packet",
            "GEARSHIFT_RESUMED <json>",
            "同一 Shift ID 且 `restored=true`",
            "armed + packet none",
            "重新生成最终 packet",
            "禁止新 Shift ID",
    ...
    require(
        ...
        (
            ...
            "final report 不作为 Projection 更新依据",
            "Shift Record 是独立真相源",
            "disabled v3 row 保留实际 mode",
...
def check_packets() -> None:
    ...
    require(
        ...
        (
            ...
            "legacy v2 replacement 固定写 mode=none、enabled=false、eligibility=none",
            "Gearshift opt-in label JSON：<quoted-label | none>",
            "最终 packet 仅在 Armed Projection readback 后生成",
